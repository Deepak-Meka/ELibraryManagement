...
namespace ELibraryManagement
{
    public partial class BookInventory : System.Web.UI.Page
    {
        ...
        {
            if (checkBookExistById())
            { 
                getBookbyId();
            }
            else
            {
                Response.Write("<script>alert('Book ID not matches with the records.');</script>");
            }
        }
        //add buttton
        ...
        void deleteBookById()
        {
            ...
            {
                string command = "Delete from  book_master_tbl where book_id='" + TextBox1.Text.Trim() + "'";
        ...
        void getBookbyId()
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string command = "select * from book_master_tbl where book_id='" + TextBox1.Text.Trim() + "';";
                SqlCommand cmd = new SqlCommand(command, con);
                con.Open();
        ...
        bool chechBookExist()
        {
            using (SqlConnection con = new SqlConnection(strcon))
            {
                string command = "select * from book_master_tbl where book_id='" + TextBox1.Text.Trim() + "' OR book_name='"+TextBox2.Text.Trim()+"';";
                SqlCommand cmd = new SqlCommand(command, con);
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
        ...
        void authorPublihserId()
        {
            using (SqlConnection connect = new SqlConnection(strcon))
            {
                string command1 = "SELECT author_name from author_master_tbl ";
                connect.Open();
                SqlCommand cmd = new SqlCommand(command1, connect);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                ...
                string command2 = "SELECT publisher_name from publisher_master_tbl ";
               
                cmd = new SqlCommand(command2, connect);
            ...
            void addNewBook()
            {
                ...
                using (SqlConnection connect = new SqlConnection(strcon))
                {
                    string command = "INSERT into book_master_tbl values(" +
                    ...
                    cmd.Parameters.AddWithValue("@book_name", TextBox2.Text.Trim());
                    cmd.Parameters.AddWithValue("@genre", generes);
                    cmd.Parameters.AddWithValue("@author_name", DropDownList3.SelectedItem.Value);
                    cmd.Parameters.AddWithValue("@publisher_name", DropDownList2.SelectedItem.Value);
                    cmd.Parameters.AddWithValue("@publish_date", TextBox3.Text.Trim());
                    cmd.Parameters.AddWithValue("@language", DropDownList1.SelectedItem.Value);
